local _, unitClassEn = UnitClass("player");

local isEpClass = false;
if (unitClassEn == "ROGUE"
        or unitClassEn == "DRUID") then
    isEpClass = true;
end

local isMpClass = false;
if (unitClassEn == "MAGE"
        or unitClassEn == "PRIEST"
        or unitClassEn == "WARLOCK"
        or unitClassEn == "DRUID"
        or unitClassEn == "HUNTER"
        or unitClassEn == "SHAMAN"
        or unitClassEn == "PALADIN") then
    isMpClass = true;
end
...
local function findManaPulseProgress(data, justAfterCastSucc)
    ...
    if (mana > data.mana) then
        ...
        -- GetManaRegen() gives 0.00xxx within 5s after cast
        local baseRegen, castRegen = GetManaRegen();
        local pulseAmount = baseRegen * PULSE_INTERVAL;
        if (diff > pulseAmount - 1 and diff < pulseAmount + 1) then
            data.pulseTime = now;
        end
    elseif (justAfterCastSucc and mana < data.mana) then
...
f:SetScript("OnUpdate", function(self, elapsed)
    ...
    elseif (powerType == Enum.PowerType.Mana) then
        local manaProgress = findManaPulseProgress(self.manaData);
        self.spark:ClearAllPoints();
